...
![菜单栏状态（模拟数据）](docs/images/menu-bar.zh-Hans.png)
...
- 主窗口账号卡会显示可重置额度：普通模式列出每一次期限，`极简` 模式则以 `可重置 2 次 · 7/30, 8/1` 这类精简日期显示。
...
- 提供菜单栏工具：紧凑账号列表、分组筛选、方案徽章、警示弹出提示，以及可重置次数/逐次期限。
...
- 展开内容会沿用主窗口的账号排序设置，包括当前账号置顶、付费账号优先、API Key 账号最后。
- 账号列表支持分组筛选，显示 Plus/Pro/API Key 徽章，且当前账号仍保留 `切换` 按钮。
- 账号行会显示周用量与 5 小时用量的剩余比例，并在可取得时显示精确重置时间。
- 可重置额度会直接展开成 `可重置 N 次` 与每一次期限。程序会优先使用 API 返回的 `expires_at`；明细请求失败或个别日期缺失时，才以“前一次成功同步时间 + 30 天”估算并保留警告提示。
- 账号警示收纳在紧凑惊叹号弹出提示中，减少垂直空间占用。
...
- 版本或 build 变化后会显示本地化“新功能”提示，也可以从设置重新查看最新功能说明。
