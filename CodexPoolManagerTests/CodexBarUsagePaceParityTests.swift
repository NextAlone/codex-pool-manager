...
struct CodexBarUsagePaceParityTests {
    @Test
    func `weekly pace computes delta and eta`() {
        let now = Date(timeIntervalSince1970: 0)
        let window = CodexBarRateWindow(
            usedPercent: 50,
            windowMinutes: 10080,
            resetsAt: now.addingTimeInterval(4 * 24 * 3600),
            resetDescription: nil)

        let pace = CodexBarUsagePace.weekly(window: window, now: now)
    ...
    func `weekly pace speed headroom uses remaining burn capacity`() throws {
        ...
        let pace = try #require(CodexBarUsagePace.weekly(window: window, now: now))
    ...
    func `historical pace speed headroom uses projected remaining usage`() {
        let pace = CodexBarUsagePace.historical(
    ...
    func `weekly pace hides when reset missing or outside window`() {
        let now = Date(timeIntervalSince1970: 0)
        let missing = CodexBarRateWindow(
            usedPercent: 10,
            windowMinutes: 10080,
            resetsAt: nil,
            resetDescription: nil)
        let tooFar = CodexBarRateWindow(
            usedPercent: 10,
            windowMinutes: 10080,
            resetsAt: now.addingTimeInterval(9 * 24 * 3600),
            resetDescription: nil)

        #expect(CodexBarUsagePace.weekly(window: missing, now: now) == nil)
        #expect(CodexBarUsagePace.weekly(window: tooFar, now: now) == nil)
    }
    ...
    func `workday aware pace shows on track for five day user on friday`() throws {
        ...
        let pace7 = try #require(CodexBarUsagePace.weekly(window: window, now: now, workDays: nil))
        let pace5 = try #require(CodexBarUsagePace.weekly(
            window: window,
            now: now,
            workDays: 5,
            calendar: calendar))
    ...
    func `workday aware exhausted quota does not last through weekend`() throws {
        ...
        let pace = try #require(CodexBarUsagePace.weekly(
            window: window,
            now: now,
            workDays: 5,
            calendar: calendar))
    ...
    func `workday aware pace stays flat on non workdays`() throws {
        ...
        let saturdayPace = try #require(CodexBarUsagePace.weekly(
            window: window,
            now: saturday,
            workDays: 5,
            calendar: calendar))
        let sundayPace = try #require(CodexBarUsagePace.weekly(
            window: window,
            now: sunday,
            workDays: 5,
            calendar: calendar))
    ...
    func `zero usage becomes safe only after the first configured workday begins`() throws {
        ...
        let before = try #require(CodexBarUsagePace.weekly(
            window: window,
            now: firstWorkday.addingTimeInterval(-1),
            workDays: 5,
            calendar: calendar))
        let boundary = try #require(CodexBarUsagePace.weekly(
            window: window,
            now: firstWorkday,
            workDays: 5,
            calendar: calendar))
        let after = try #require(CodexBarUsagePace.weekly(
            window: window,
    ...
    func `workday aware pace falls back to linear when workDays is nil or 7`() throws {
        ...
        let paceNil = try #require(CodexBarUsagePace.weekly(window: window, now: now, workDays: nil))
        let pace7 = try #require(CodexBarUsagePace.weekly(window: window, now: now, workDays: 7))
        let paceDefault = try #require(CodexBarUsagePace.weekly(window: window, now: now))
    ...
    func `workdays off linear weekly pace keeps deficit sign`() throws {
        ...
        let pace = try #require(CodexBarUsagePace.weekly(window: window, now: now, workDays: nil))
    ...
    func `workday aware pace ignores non weekly windows`() throws {
        ...
        let paceNoWork = try #require(
            CodexBarUsagePace.weekly(window: window, now: now, defaultWindowMinutes: 300, workDays: nil))
        let paceWork5 = try #require(
            CodexBarUsagePace.weekly(window: window, now: now, defaultWindowMinutes: 300, workDays: 5))
    ...
    func `session pace computes delta and eta for five hour window`() {
        ...
        let pace = CodexBarUsagePace.weekly(window: window, now: now, defaultWindowMinutes: 300)
    ...
    func `one work day falls back to linear pace`() throws {
        ...
        let paceOne = try #require(CodexBarUsagePace.weekly(window: window, now: now, workDays: 1))
        let paceNil = try #require(CodexBarUsagePace.weekly(window: window, now: now))
...
struct CodexBarMenuPaceEligibilityTests {
    @Test func displayMatchesCodexBarWindowAndCapacityGates() throws {
        let now = Date(timeIntervalSince1970: 0)
        func display(used: Double, remaining: TimeInterval?, minutes: Int = 10080) -> CodexBarPacePresentation? {
            CodexBarPacePresentation.make(usedPercent: used, resetsAt: remaining.map { now.addingTimeInterval($0) },
                windowMinutes: minutes, now: now, isSession: minutes == 300)
        }
        #expect(display(used: 50, remaining: nil) == nil)
        #expect(display(used: 50, remaining: -1) == nil)
        #expect(display(used: 50, remaining: 8 * 86400) == nil)
        #expect(display(used: 50, remaining: 6.9 * 86400) == nil)
        #expect(display(used: 100, remaining: 4 * 86400) == nil)
        let deficit = try #require(display(used: 50, remaining: 4 * 86400))
        #expect(!deficit.isInReserve)
        #expect(abs((deficit.expectedRemainingPercent ?? 0) - 57.142857) < 0.001)
        let reserve = try #require(display(used: 5, remaining: 4 * 86400))
        #expect(reserve.isInReserve)
        #expect(display(used: 50, remaining: 17800, minutes: 300) == nil)
        #expect(display(used: 50, remaining: 7200, minutes: 300) != nil)
    }

    @Test func failedOrExcludedAccountHidesPace() {
        let now = Date(timeIntervalSince1970: 0)
        var account = AgentAccount(id: UUID(), name: "test", usedUnits: 50, quota: 100,
            usageWindowName: "weekly", usageWindowResetAt: now.addingTimeInterval(4 * 86400))
        #expect(MenuBarDashboardPresenter.usageWindows(for: account, now: now).first?.pace != nil)
        account.usageSyncError = "failed"
        #expect(MenuBarDashboardPresenter.usageWindows(for: account, now: now).first?.pace == nil)
        account.usageSyncError = nil
        account.isUsageSyncExcluded = true
        #expect(MenuBarDashboardPresenter.usageWindows(for: account, now: now).first?.pace == nil)
    }

    @Test func countdownUsesCodexBarRoundingAndUnits() {
        #expect(CodexBarCountdown.duration(seconds: 0) == "now")
        #expect(CodexBarCountdown.duration(seconds: 1) == "1m")
        #expect(CodexBarCountdown.duration(seconds: 3600) == "1h")
        #expect(CodexBarCountdown.duration(seconds: 86401) == "1d 1m")
        #expect(CodexBarCountdown.duration(seconds: 90060) == "1d 1h")
    }
}
